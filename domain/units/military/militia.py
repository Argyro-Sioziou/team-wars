from domain.units.base.unit import Unit, UnitStats

class Militia(Unit):
    """
    Militia Unit - Basic defensive unit available from Barracks level 1
    
    A basic defensive unit that provides early-game city protection.
    Cost-effective for defensive strategies but limited offensive capabilities.
    """
    
    def __init__(self):
        stats = UnitStats(
            attack_power=5,
            defense_power=10,
            health_points=50,
            speed=3  # Slow speed value
        )
        
        super().__init__(
            training_cost=50,  # 50 Gold per unit
            training_time=60,  # 1 minute = 60 seconds
            food_requirement=1,  # 1 Food per unit
            stats=stats,
            barracks_level_required=1
        )
    
    @property
    def unit_type(self) -> str:
        return "Militia"
    
    @property
    def description(self) -> str:
        return "Basic defensive unit with high defense but limited attack capabilities."
    
    def to_dict(self):
        base_dict = super().to_dict()
        return {
            **base_dict,
            "unit_type": self.unit_type,
            "description": self.description
        } 